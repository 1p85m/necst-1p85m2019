...
class optical_pointing(object):
    ...
    def f_az(self, X, a1, a2, a3):
        Az, El = X
        return a1*np.sin(El*(np.pi/180.))/np.cos(El*(np.pi/180.)) + a2/np.cos(El*(np.pi/180.)) + a3 + self.b1*np.sin(Az*(np.pi/180.))*np.sin(El*(np.pi/180.))/np.cos(El*(np.pi/180.)) - self.b2*np.cos(Az*(np.pi/180.))*np.sin(El*(np.pi/180.))/np.cos(El*(np.pi/180.))
    ...
    def fitting(self):
        ...
        print(111)
        fit_dEl = curve_fit(self.f_el, (Az, El), dEl,check_finite=False,p0=[0,0,0,0])
        self.b1 = fit_dEl[0][0]
        self.b2 = fit_dEl[0][1]
        b3 = fit_dEl[0][2]
        g1 = fit_dEl[0][3]
